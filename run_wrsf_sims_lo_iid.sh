#!/bin/bash
#SBATCH --job-name=wrsf_sims_li    # name of the job
...
if [ -f results/wrsf_sim_results_lo_iid.csv ]; then
	echo "Results file already exists! continuing..."
else
	echo "creating results file wrsf_sim_results_lo_iid.csv"
	echo "sim_no,samp_freq,iid_coef,iid_lcl,iid_ucl,runtime" > results/wrsf_sim_results_lo_iid.csv
fi

Rscript wrsf_sims_lo_iid.R ${SLURM_ARRAY_TASK_ID}     # name of script
