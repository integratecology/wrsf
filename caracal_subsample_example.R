...
df <- read.csv("data/caracal_example.csv")

sample <- seq(sim_no, length(df$timestamp), by=round(length(df$timestamp)/18.8)) # ESS = 18.8
...
rsf_iid <- ctmm:::rsf.fit(l, UD=ud_iid, R=list(plantation=plantation,urban=urban), 
                          debias=TRUE, error=0.01, integrator="Riemann")
summary(rsf_iid)
...
# Vector of resultsn
x <- data.frame(species,iid_pest,iid_plcl,iid_pucl,iid_uest,iid_ulcl,iid_uucl)

# Store results in data.frame
write.table(x, 'results/caracal_subsample_results.csv', append=TRUE, row.names=FALSE, col.names=FALSE, sep=',') 
