...
sample <- seq(sim_no, length(df$timestamp), by=round(length(df$timestamp)/109.6)) # ESS = 109.6
...
write.table(x, 'results/serval_subsample_results.csv', append=TRUE, row.names=FALSE, col.names=FALSE, sep=',') 
