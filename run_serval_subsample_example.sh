...
#SBATCH --array=1-33%33    	   # set number of total simulations and number that can run simultaneously	
...
if [ -f results/serval_subsample_results.csv ]; then
        echo "Results file already exists! continuing..."
else
        echo "creating results file"
        echo "species,iid_pest,iid_plcl,iid_pucl,iid_uest,iid_ulcl,iid_uucl" > results/serval_subsample_results.csv
fi

Rscript serval_subsample_example.R ${SLURM_ARRAY_TASK_ID} # name of script
